...
const ActiveCourses = () => {
  ...
  // ================= LOAD WATCHED MODULES =================
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("watchedModules")) || [];
    setWatchedModules(saved);
  }, []);
  ...
  const watchVideo = (module) => {
    markAsWatched(module._id);
    navigate("/user/video", {
      state: { youtubeUrl: module.youtubeUrl },
    });
  };
        ...
        <div className="relative overflow-hidden bg-black text-white">
          ...
          <div className="relative px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-14">
            ...
            <button
              ...
              className="flex items-center gap-1.5 sm:gap-2 bg-white/10 hover:bg-white/20 border border-white/10 px-3 py-2 sm:px-5 sm:py-3 rounded-xl sm:rounded-2xl transition-all duration-300 backdrop-blur-md text-sm sm:text-base"
            >
            ...
            <div className="mt-6 sm:mt-10 flex flex-col xl:flex-row gap-6 sm:gap-10 items-start justify-between">
              ...
              <div className="w-full xl:w-[300px] bg-white/10 border border-white/10 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6">
                <h3 className="text-base sm:text-xl font-bold mb-3 sm:mb-6">Course Information</h3>
                <div className="space-y-3 sm:space-y-5">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <CheckCircle2 size={14} className="text-green-400 sm:w-[18px] sm:h-[18px]" />
                      <span className="text-sm sm:text-base">Completed</span>
                    </div>
                    <span className="font-semibold text-green-300 text-sm sm:text-base">
                      {courseData?.modules?.filter((m) => watchedModules.includes(m._id)).length} /{" "}
                      {courseData?.modules?.length || 0}
                    </span>
                  </div>
                </div>
        ...
        <div className="px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10">
          <div className="flex items-center justify-between mb-5 sm:mb-8 flex-wrap gap-3 sm:gap-4">
            ...
            <div className="bg-white border border-zinc-200 rounded-xl sm:rounded-2xl px-3 py-1.5 sm:px-5 sm:py-3 shadow-sm">
              <p className="text-[10px] sm:text-sm text-zinc-500">Learning Progress</p>
          ...
          <div className="space-y-3 sm:space-y-5">
            {courseData?.modules?.length > 0 ? (
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6">
                      <div className="flex items-start gap-3 sm:gap-4">
                        <div
                          className={`
                            ...
                            ${watched ? "bg-green-600 text-white" : "bg-black text-white"}
                          `}
                        >
                          {watched ? <CheckCircle size={18} className="sm:w-7 sm:h-7" /> : i + 1}
                        </div>
                        <div>
                      ...
                      <button
                        ...
                        className={`
                          ...
                          rounded-xl sm:rounded-2xl transition-all duration-300 w-full lg:w-auto text-sm sm:text-base
                          ${watched ? "bg-green-600 hover:bg-green-700 text-white" : "bg-black hover:bg-zinc-800 text-white"}
                        `}
                      >
                        {watched ? (
                          <>
              ...
              <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-10 text-center border border-zinc-200">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-zinc-900">No Modules Available</h3>
                <p className="text-zinc-500 text-sm sm:text-base mt-2">Admin has not uploaded modules yet</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-4 sm:gap-8">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white rounded-2xl sm:rounded-3xl h-[280px] sm:h-[350px] animate-pulse" />
            ))}
          </div>
        ) : courses.length === 0 ? (
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-zinc-200 p-6 sm:p-10 lg:p-16 text-center">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-zinc-900">No Active Courses</h2>
            <p className="text-zinc-500 text-sm sm:text-base mt-2 sm:mt-4 max-w-xl mx-auto">
              Once admin approves your enrollment request, your active courses will appear here.
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="p-3 sm:p-6">
                  ...
                  <div className="mt-3 sm:mt-6 flex flex-col sm:flex-row gap-2 sm:gap-3">
                    ...
                    <div className="sm:w-[100px] rounded-xl sm:rounded-2xl bg-green-50 border border-green-200 flex flex-col items-center justify-center py-1.5 sm:py-3">
                      <span className="text-[9px] sm:text-xs text-green-600 font-medium">Progress</span>
                      <span className="text-sm sm:text-xl font-black text-green-700">
                        {item.courseId?.modules?.length
                          ? `${Math.round(
                              (watchedModules.filter((id) =>
                                courseData?.modules?.some((m) => m._id === id)
                              ).length /
                                item.courseId.modules.length) *
                                100
                            )}%`
                ...
                <div className="absolute inset-0 rounded-2xl sm:rounded-[32px] border border-transparent group-hover:border-zinc-300 transition-all duration-500 pointer-events-none" />
              </div>
            ))}
          </div>
        )}
